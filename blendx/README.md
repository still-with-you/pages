# TACT Project Page

Project page for “Beyond Task-Oriented and Chitchat Dialogues: Proactive and Transition-Aware Conversational Agents”.

## Links
- Project page: https://still-with-you.github.io/tact-page/
- Paper (ACL Anthology): https://aclanthology.org/2025.emnlp-main.672/
- arXiv: https://arxiv.org/abs/2511.08835
- Code: https://github.com/HYU-NLP/TACT
- Dataset: https://huggingface.co/datasets/HYU-NLP/TACT

## Citation
See `index.html` for BibTeX.

## Template & License
- Template: https://github.com/eliahuhorwitz/Academic-project-page-template (adapted from https://nerfies.github.io/)
- License: https://creativecommons.org/licenses/by-sa/4.0/
